...
float area_holding, area_shortage, holding_cost, incremental_cost, maxlag,
    ...
    total_ordering_cost;
float time_backlog, num_express;
FILE *infile, *outfile;
...
int main() /* Main function. */
{
    ...
    outfile = fopen("inv_express.out", "w");
...
void evaluate(void) /* Inventory-evaluation event function. */
{
    ...
    if (inv_level < smalls)
    {
        ...
        /* check an express order at initial state */
        if (inv_level < 0)
        {
            ++num_express;
            amount = bigs - inv_level;
            total_ordering_cost += 48 + 3 * amount;
            time_next_event[1] = sim_time + uniform(0.25, 0.50);
        }
        else
        {
...
void report(void) /* Report generator function. */
{
    ...
    fprintf(outfile, "\n\n(%3d,%3d)%15.2f%15.2f%15.2f%15.2f%15.2f%15.2f",
            smalls, bigs,
            avg_ordering_cost + avg_holding_cost + avg_shortage_cost,
            avg_ordering_cost, avg_holding_cost, avg_shortage_cost,
            time_backlog / sim_time, num_express / num_months);
}
                                     ...
                                     statistics. */
{
    ...
    if (inv_level < 0)
    {
        ...
        time_backlog += time_since_last_event;
    }
    else if (inv_level > 0)
